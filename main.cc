...
unsigned int gamespeed; //Used later in the timing of the main loop. Defaults at 50, change up with "," down with "."
...
SDL_Surface *food;      //Surface containing all of the food objects in the order that they appear in the enum. 32h x (32*5)w
...
int main(int argc, char *argv[])
{
  ...
  srand(static_cast<unsigned>(time(0)));//Seed the number generator with the time so that the numbers will be different every time the game is run.
  //printf("Like, time(0) is, uhhhh: %u", static_cast<unsigned>(time(0)));
  if (initSDL() == false){  //If Initializing SDL didn't work for some reason, then don't go into the mainloop and just exit.
  mainloop();
  }
  return 0;
}
...
bool initSDL()
{ 
  if (SDL_Init(SDL_INIT_VIDEO) != 0) {   //If SDL Video Initialization fails, then return true to main() so it won't go on with mainloop() and report the error for debugging.
...
void newfood(char index)//A void that takes the index of a food obj in the array and (re)creates it, randomizing everything.
{
  fallobj[index].type = (char)random(1, 5);           //Set the type randomly.
  fallobj[index].velocity = fallobj[index].type;      //Set the velocity accordingly to the type. Cakes are harder to catch than lollipops, and so give more points.
  fallobj[index].xpos = random(1, SCREEN_WIDTH - 32); //Set the x location randomly, making sure that it isn't off of the picture.
  fallobj[index].ypos = 0;                            //Set the y location to 0, so that it can start from the top and give the player a fair chance of catching it.
  fallobj[index].inited = true;                       //Set whether the object has been initialized, so that the main loop can know whether to use it.  
}
...
{ 
  ...
  brunning = true;    //When it is set to true the loop continues. When false, it stops and passes control back to main to quit.
  ...
  bool upup = true;                //A set of booleans used in the input loop so the keydown case knows whether the key has been up or not
  bool downup= true;
  bool leftup= true;
  bool rightup= true;
  ...
  { 
    while (SDL_PollEvent(&keyevent))   //Poll our SDL key event for any keystrokes.
    {
      switch (keyevent.type)
      {
        ...
        case SDL_KEYDOWN:
          switch (keyevent.key.keysym.sym)
          {
            ...
            case SDLK_h:
              ...
              break;
            case SDLK_UP:
              catchyvel += -1;//When the user presses the arrow keys, adjust the velocities of the catcher mouth accordingly and set the keyup boolean so that we can continue to move the mouth until there is a keyup event.  
              upup = false;
              break;
            case SDLK_DOWN:
              catchyvel += 1; 
        ...
        case SDL_KEYUP:
          ...
          {
            case SDLK_UP:
              if (!rubbermode)catchyvel = 0;  //When the keys are let up, set the velocities accordingly and change the keyup booleans so that it will know that the keys aren't down anymore.
              upup = true;
              break;
            case SDLK_DOWN:
              if (!rubbermode) catchyvel = 0; 
              downup = true;
              break;
            case SDLK_RIGHT:
              if (!rubbermode)catchxvel = 0;
              rightup = true;
              break;
            case SDLK_LEFT:
              if (!rubbermode)catchxvel = 0;
    ...
    if (!upup) catchyvel -= 1;  //These continue to accelerate the mouth if the user is holding down a button.
    if (!downup) catchyvel += 1; //They have to use these booleans and be out here because no matter how long you hold down the key, it only generates two events: down and up.
    if (!rightup) catchxvel += 1;
    if (!leftup) catchxvel -= 1;
    ...
    for (char x=0; x<=5; x++)    //Loop through all the food objects and do things to them.
    {
      if (fallobj[x].inited == false) {
        ...
        if (fallobj[x].ypos + fallobj[x].velocity <= SCREEN_HEIGHT){  //If the result of a drop down doesn't put the food below the screen.
          fallobj[x].ypos += fallobj[x].velocity;  //Make the food object fall down the screen at its individual velocity.    
          if (((fallobj[x].xpos >= catcherxpos) && (fallobj[x].xpos <= catcherxpos + 64)) && ((fallobj[x].ypos >= catcherypos) && (fallobj[x].ypos <= catcherypos + 64))) {
          ...
          fallobj[x].inited = false; // If the result of a drop down puts the food below the screen, then uninit the food so it will be reinitialized on the next iteration of the loop. 
        }
      }
    }
    ...
    looptime = SDL_GetTicks();   //Store time of completion of main loop so the next iteration can so this timing thing. 
  }
}

void render()
{
  drawimage(bg, 0 , 0, SCREEN_WIDTH, SCREEN_HEIGHT, screen, 0, 0, 255);          //Draw the background onto the screen before anything else.
  for (char x=0; x<=5; x++){
    drawimage(food, fallobj[x].type * 32 - 32, 0, 32, 32,screen, fallobj[x].xpos, fallobj[x].ypos, 255);//For every food item, draw it on the screen.     
  }    
  drawimage(mouth, 0, 0, 64, 64, screen, catcherxpos, catcherypos, 255);         //Draw the catcher at the proper location on the screen.
...
void updatescore()
{
  ...
  char boundstext[6];
  if (boundsmode) sprintf(boundstext, "True"); else sprintf(boundstext, "False");//Make the strings with thrue and false for the modes.
  if (rubbermode) sprintf(rubbertext, "True"); else sprintf(rubbertext, "False");
  sprintf(fonttext, "\"Eat\" by Snoolas: Score: %d, Speed: %d, Bounds: %s, Rubber: %s", score, gamespeed, boundstext, rubbertext); //The text is put together in fontext by sprintf, then rendered.
...
int random(int lowest_number, int highest_number)      //This function returns a pseudo-random number between the parameters.
{
